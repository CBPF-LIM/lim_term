...
class GraphTab:
    ...
    def _create_options_widgets(self):
        ...
        self.window_label.grid(column=0, row=0, padx=5, pady=5, sticky="w")
        self.data_window_entry = ttk.Entry(global_frame, width=12)
        self.data_window_entry.grid(column=1, row=0, padx=5, pady=5, sticky="w")
        ...
        # Min and Max Y (global for all series)
        self.min_y_label = ttk.Label(global_frame, text=t("ui.graph_tab.min_y_label"))
        self.min_y_label.grid(column=2, row=0, padx=5, pady=5, sticky="w")
        self.min_y_entry = ttk.Entry(global_frame, width=12)
        self.min_y_entry.grid(column=3, row=0, padx=5, pady=5, sticky="w")
        ...
        self.max_y_label.grid(column=4, row=0, padx=5, pady=5, sticky="w")
        self.max_y_entry = ttk.Entry(global_frame, width=12)
        self.max_y_entry.grid(column=5, row=0, padx=5, pady=5, sticky="w")
        self.max_y_entry.bind("<KeyRelease>", self._on_setting_change)
        self.max_y_entry.bind("<FocusOut>", self._on_preference_changed)
        
        # Per-series settings
        series_frame = ttk.LabelFrame(self.options_frame, text="Series Settings")
        series_frame.grid(column=0, row=1, columnspan=6, padx=5, pady=5, sticky="ew")
        
        # Headers
        ttk.Label(series_frame, text="Series").grid(column=0, row=0, padx=5, pady=5)
        ttk.Label(series_frame, text=t("ui.graph_tab.type_label")).grid(column=1, row=0, padx=5, pady=5)
        ttk.Label(series_frame, text=t("ui.graph_tab.color_label")).grid(column=2, row=0, padx=5, pady=5)
        ttk.Label(series_frame, text=t("ui.graph_tab.point_label")).grid(column=3, row=0, padx=5, pady=5)
        
        # Y1-Y5 series only
        self.series_widgets = []
        for i in range(1, 6):
            self._create_series_row(series_frame, i, f"Y{i}", i-1)
