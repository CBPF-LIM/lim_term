...
class GraphManager:
    ...
    def plot_multi_series(
        ...
        if settings_list:
            ...
            min_y = (
                float(first_settings.get("min_y", "0"))
                if first_settings.get("min_y")
                else None
            )
            max_y = (
                float(first_settings.get("max_y", "0"))
                if first_settings.get("max_y")
                else None
            )
    ...
    def plot_stacked_series(
        ...
        if normalize_100:
            ...
            for i in range(min_length):
                ...
                if total == 0:
                    total = 1
                normalized_y_lists.append(
                    [y_list[i] / total * 100 for y_list in y_lists]
                )
